...
//	1일부터 달력을 출력할 달의 마지막 날짜까지 반복하며 날짜를 출력
	for (int i = 1; i <= MyCalendar.lastDay(year, month); i++){
		...
		if(month == 1 && i == 1){
			...
			if(MyCalendar.weekDay(year, 3, 1) == 0 || MyCalendar.weekDay(year, 3, 1) == 6){
				rep += MyCalendar.weekDay(year, 3, 1) / 6 + 2;
			}
			...
			if(MyCalendar.weekDay(year, 5, 5) == 0 || MyCalendar.weekDay(year, 5, 5) == 6){
				rep += MyCalendar.weekDay(year, 5, 5) / 6 + 2;
			}
			...
			if(MyCalendar.weekDay(year, 8, 15) == 0 || MyCalendar.weekDay(year, 8, 15) == 6){
				rep += MyCalendar.weekDay(year, 8, 15) / 6 + 2;
			}
			out.println("<td class='holiday'>" + i + "<br><span>광복절</span></td>");	
		} else if(month == 10 && i == 3){
			if(MyCalendar.weekDay(year, 10, 3) == 0 || MyCalendar.weekDay(year, 10, 3) == 6){
				rep += MyCalendar.weekDay(year, 10, 3) / 6 + 2;
			}
			out.println("<td class='holiday'>" + i + "<br><span>개천절</span></td>");	
		} else if(month == 10 && i == 9){
			if(MyCalendar.weekDay(year, 10, 9) == 0 || MyCalendar.weekDay(year, 10, 9) == 6){
				rep += MyCalendar.weekDay(year, 10, 9) / 6 + 2;
			}
			out.println("<td class='holiday'>" + i + "<br><span>한글날</span></td>");	
		} else if(month == 12 && i == 25){
			if(MyCalendar.weekDay(year, 12, 25) == 0 || MyCalendar.weekDay(year, 12, 25) == 6){
				rep += MyCalendar.weekDay(year, 12, 25) / 6 + 2;
			}
		...
		// 공휴일을 제외한 나머지 날짜
			...
			//	<td> 태그에 요일에 따른 class 속성을 지정
			/*
			if (MyCalendar.weekDay(year, month, i) == 0) { // 일요일인가
				out.println("<td class='sun'>" + i + "</td>");
			} else if (MyCalendar.weekDay(year, month, i) == 6) { // 토요일인가
